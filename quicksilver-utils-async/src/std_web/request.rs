...
pub async fn get_resource(url: &str) -> Result<String, RequestError> {
    ...
    xhr.open("GET", url).map_err(|e| RequestError::NativeError(format!("Open: {}", e)))?;
    xhr.set_response_type(XhrResponseType::ArrayBuffer).map_err(|e| RequestError::NativeError(format!("Set Response Type: {}", e)))?;
    xhr.send().map_err(|e| RequestError::NativeError(format!("Send: {}", e)))?;

    let xhr_closure = XhrClosure { inner: Arc::new(RefCell::new(XhrClosureInner { xhr, have_set_handlers: false }))};
    ...
    let result = poll_fn(move |ctx| {
        ...
        if !inner.have_set_handlers {
            ...
            inner.xhr.add_event_listener(move |_: ProgressLoadEvent| waker.wake_by_ref());
            let waker = ctx.waker().clone();
            inner.xhr.add_event_listener(move |_: ProgressAbortEvent| waker.wake_by_ref());
        }
        ...
        match (status / 100, ready_state) {
            (2, XhrReadyState::Done) => {
                let reference: Reference = inner.xhr
                    .raw_response()
                    .try_into()
                ...
                Poll::Ready(
                    reference
                        ...
                        .ok_or_else(|| RequestError::NativeError("Failed to cast file into bytes".to_string())),
                )
            }
            (2, _) => Poll::Pending,
            (0, _) => Poll::Pending,
            _ => Poll::Ready(Err(RequestError::NativeError("Non-200 status code returned".to_string()))),
        }
    }).await?;
